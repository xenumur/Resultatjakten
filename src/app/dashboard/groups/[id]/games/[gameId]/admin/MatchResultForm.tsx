...
export function MatchResultForm({ action, matchId, homeScore, awayScore, status }: MatchResultFormProps) {
  ...
  return (
    <form action={formAction} id={`form-${matchId}`} className="contents">
      <td className="p-4">
      ...
      <td className="p-4">
        <select name="status" defaultValue={status} className="text-sm px-3 py-2 rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-950 outline-none focus:ring-2 focus:ring-indigo-500">
      ...
      <td className="p-4 text-right">
        <div className="flex justify-end gap-2">
          <SubmitButton className="text-sm bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg font-semibold transition shadow-sm">
            Spara
          </SubmitButton>
        </div>
      </td>
