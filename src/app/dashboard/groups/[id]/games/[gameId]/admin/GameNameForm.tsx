    ...
    <div className="flex flex-col">
      ...
      <form action={action} className="flex items-center gap-2">
        <input 
          ...
          autoFocus
        />
        <SubmitButton className="p-2 h-auto text-xs bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg">
          <Check className="w-5 h-5" />
        </SubmitButton>
        <button 
          type="button" 
          onClick={() => setIsEditing(false)}
          className="p-2 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-600 dark:text-zinc-300 rounded-lg transition-colors"
        >
          <X className="w-5 h-5" />
        </button>
      </form>
